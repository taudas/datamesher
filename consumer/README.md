...
Current: mirrors the producer's bring-up, then dials the producer's TCP address, exchanges `ConnectionInfo`, and connects the queue pair through to RTS. Then parks — no producer discovery, no workload hook yet — see TODOs in `src/main.rs`.
